...
public:
	...
	int search_220909(vector<int>& nums, int target) {
		int n = nums.size();
		int l = 0;
		int r = n - 1;
		int m = 0;
	...
	/*=======================       findMaximumXOR 221026 12:17      =================================*/
	int findMaximumXOR(vector<int>& nums) {
		TrieNode_XOR* trie_node = new TrieNode_XOR();
		for (int x : nums) {
			trie_node->increase(x);
		}

		int ans = 0;
		for (int x : nums) {
			ans = std::max(ans, trie_node->find_max(x));
		}
		return ans;
	}
	/*================================================================================================*/
...
/*&&&&&&&&&&&&&&&&&&&&&&&&&&&&        Trie Node XOR 221026 12:41        &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
struct TrieNode_XOR {
	TrieNode_XOR* children[2] = {};

	void increase(int number) {
		TrieNode_XOR* cur = this;

		for (int i = 31; i >= 0; --i) {
			int bit = (number >> i) & 1;
			if (!cur->children[bit]) {
				cur->children[bit] = new TrieNode_XOR();
			}
